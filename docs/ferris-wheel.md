...
### パーツリスト（BOM・2026-07-15 更新：7/13注文分を手持ちに反映）

7/13のAmazon注文（工具・締結・配線材、**7/20〜22着**）で、ベアリング・インサート・ネジ類・配線材がカバーできることを確認済み。**追加発注が必要なのは下表の🛒行のみ**です。

| # | 品目 | 型番・仕様例 | 数量 | 状態 | 概算(税込) |
|--:|---|---|--:|---|--:|
| 1 | ウォームギヤードモーター | JGY-370 12V 2〜5rpm | 1 | 🛒 発注（電動版のみ） | ¥1,600 |
| 2 | ACアダプタ | 12V 2A（5.5×2.1mmプラグ） | 1 | 🛒 発注（電動版のみ） | ¥1,300 |
| 3 | PWM速度コントローラ | DC 1.8〜15V 2A | 1 | 🛒 発注（電動版のみ） | ¥700 |
| 4 | ロッカースイッチ＋DCジャック | パネルマウント | 1式 | 🛒 発注（電動版のみ） | ¥600 |
| 5 | ベアリング | 608 ABEC-9（内径8mm）5個入り | 2使用 | ✅ **7/13注文済**（7/20〜22着） | — |
| 6 | シャフト | Φ8mm 鋼 L250mm（Dカット） | 1 | 🛒 発注（共通） | ¥900 |
| 7 | シャフトカプラ | 6mm→8mm | 1 | 🛒 発注（電動版のみ） | ¥600 |
| 8 | LEDテープ | WS2812B 60LED/m IP30 | 2m | 🛒 発注（共通） | ¥2,200 |
| 9 | LEDコントローラ | M5 Atom Lite | 1 | ✅ 手持ち | — |
| 10 | モバイルバッテリー | 小型 5V/2A（ハブ同乗用） | 1 | ✅ 手持ち | — |
| 11 | ネジ・インサート類 | M3ヒートインサート152個／M3ネジ350個セット／M4-M6ナットボルト114個 | 1式 | ✅ **7/13注文済**（7/20〜22着） | — |
| 12 | ゴンドラ軸 | Φ3mmステンレス丸棒 L300＋Eリング | 2 | 🛒 発注（共通） | ¥600 |
| 13 | 配線・工具 | XH/PH 3ピンコネクタ230個・ジャンパーワイヤー・熱収縮チューブ・はんだステーション・ニッパー・サーボテスター（PWM確認用） | 1式 | ✅ **7/13注文済**（7/20〜22着） | — |
| | | | | **追加発注合計** | **電動版 約¥8,500 ／ 手回し版 約¥3,700** |

> 🛒分は**7/16までにPrime即納品で発注**（7/17着目標）。7/13注文分（#5・#11・#13）は**7/20〜22着**のため、7/19の仮組立ではインサート・ベアリングが未着 → スケジュールを下記のとおり調整済み。
...
| 追加発注費（7/13注文分の手持ち反映後） | **約¥8,500** | **約¥3,700**（モーター¥1,600・ACアダプタ¥1,300・PWM¥700・スイッチ¥600・カプラ¥600 ＝ ▲¥4,800） |
| 追加発注の内訳 | モーター系5点＋シャフト・LED・ゴンドラ軸 | シャフト¥900・LED¥2,200・ゴンドラ軸¥600のみ |
...
**切替判断**: 発注時（7/16）にモーター系5点の実売合計が**¥6,000を超える**場合は手回し版へ。ホイール本体の印刷はどちらでも共通なので、判断を待たずに印刷を開始できます。
