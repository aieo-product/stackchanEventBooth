...
## 進行中 🚧
- **観覧車（#12）テスト印刷→失敗→修正済み・再テスト待ち**:
  - 7/15テスト印刷でリム円弧が**スパゲッティ化**（原因=スポーク/ボス/テノンが4mm浮いた設計・タイムラプス解析で特定）、ゴンドラは**箱型で搭乗機が見えない**問題
  - **修正完了（#18 / PR #20 マージ待ち）**: リム接地化＋check_dims.pyに接地チェック恒久追加＋**チェア型オープンゴンドラ**（2段ポケット床: 内48.6×56.6=公式K151ベース48.0×56.0実寸嵌合・外62×62=タカオ版汎用+ゲルテープ／前面Dバーのみで全身可視）
  - **再テスト印刷待ち**: 修正版rim-segment＋新gondola（Bambu Studioに展開済み）。K151実機で嵌合確認
- **ミニ観覧車（#19・Artec木製キット055521）**: アトムニャン/ミニマル搭乗アダプタ2種をCAD済み（PR #20に同梱）。**差込タブ等4点の実測待ち**（issueに依頼リスト）。3Vモーター駆動は対角配置で釣合い→回る見込み
- 調達: Amazon3点発注済（シャフト7/18着）／秋月SK6812×2=店頭／Bambuフィラメント7本発注済7/16（**グラディエント売切→ゴンドラは手持ちパステル4色**・経費計上はPLAウッド¥3,042のみ）
- 教室小物: 単品ミニチュア9点採用（教室シーン2750405は一体成形で不採用）＋100均装飾20点リンク付きリスト（research/hyakkin-props-20260716.md）
- 印刷スケジュール: リム量産は**PR #20マージ→再テスト成功後**に開始（/ferris-wheel の表は7/16開始想定のため1日繰り下げ濃厚。7/23予備日で吸収可）
