...
## 今いちばん大事な状態 🔥
- **印刷キュー(残り10h・全て単色)**: ①リム残り(1個1.3h・自宅は計3個でOK=応援4＋白1号) → ②base×2(高速設定可: 0.28レイヤー/インフィル8%) → ③hub×2＋crank(実績設定) → ④LT会最小セット(ソケットタイル×2＋wall-screen-l/r＋podium) → ⑤余力でstage・窓壁・通常壁・残タイル
...
- **Φ2.9セルフタップ穴は「棒を押しても入らない」のが正常** — M3ネジをドライバーで回して食い込ませる(ユーザー理解済み)。全く食い込まない場合のみΦ3.1化→hub再印刷(2h)

## 観覧車(v6・8b25bc4まで)
- 駆動: 8mm鉄シャフト→ハブΦ8.2(イモネジM3×2/Φ2.9直タップ)→支柱上端608ZZ(Φ22.2ポケット・千石調達)→クランク。ゴンドラ吊りΦ6印刷軸+キャップ(検証済)
...
- 差し込み式: ソケット付床タイル×4(+Y縁カーブh13・タブ係合12mm)に壁を直差し。スクリーンはL/R分割→半厚ダブテール連結で300mm幅、**紙288×117(A4横)が2枚跨ぎで上から差さる**。窓壁(左)・通常壁(右)・演台(ロゴ紙43×25)・ステージ台
