...
- **観覧車（#12/#18）: 修正版CAD確定・本番印刷開始待ち**
  ...
  - 7/19リグテスト→2件NG（ブリッジが搭乗経路を塞ぐ/軸が穴に入らない）→**v3修正済み(e1d487e)**: 両腕フックタワー化（天面オープン・真上搭乗）＋公差拡大（穴6.8/J溝6.7/キャップ6.0/先端テーパー）
  - **教訓: 搭乗系は着座干渉に加え搭乗動線(loading path)の検証必須**
  - 次: v3再テスト（gondola/axle/rig-disc再印刷・Bambu Studio展開済み）→ OKならリム量産開始
