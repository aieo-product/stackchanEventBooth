...
## 進行中 🚧
- **観覧車（#12）/ 撮影ブース3Dプリント化（#13）**: PR #14/#15 **マージ済み・本番デプロイ済み**。設計はページ参照（/ferris-wheel, /photobooth）
  - 完成予想図（AI生成CG）を両ページに掲載済み
  - **電動 vs 手回し比較あり**: モーター系5点の実売が¥6,000超なら手回しクランク版へ（約¥9,800→約¥5,000）。切替判断=7/16発注時
- 追加調査: `research/makerworld-props-20260715.md`（学校小物/汎用小物/観覧車 計26件）
- 物理作業の期限: **電装発注7/16**・CAD→7/19仮組立・印刷は7/15〜7/23で観覧車50h+ブース25h+アクセ優先20h≒95h（1日12h立ち会い枠・日別表は/ferris-wheel）
- プランB: CADが7/18までに間に合わなければ espRyk電動観覧車(makerworld 911733)＋ミニスタックチャン搭乗へ切替
